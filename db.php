...
/*
 * MongoDB database proxy class
 */

if (class_exists("Mongo")) {
    
    require_once(WP_CONTENT_DIR.'/plugins/wp-mongodb/PHP-SQL-PARSER/php-sql-parser.php');
    ...
    <p>If you don\'t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="http://wordpress.org/support/">WordPress Support Forums</a>.</p>' ), htmlspecialchars( $db, ENT_QUOTES ), htmlspecialchars( $this->dbuser, ENT_QUOTES ) ), 'db_select_fail' );
            ...
            function db_connect() {
                    ...
                    try {
                        $this->mongo['conn'] = new Mongo(WP_MONGODB_IP);
                        $mongodb = $this->dbname;
                        $conn = $this->mongo['conn'];
                        $this->mongo['db'] = $conn->$mongodb;
                        echo '<pre>'; print_r($this->mongo); echo '</pre>';
                    } catch(Exception $ex) {
                        // fall through
                    }
    ...
    $wpdbMongo = new wpdb_mongo(DB_USER,DB_PASSWORD,DB_NAME,DB_HOST);
    $wpdbMongo->wpdb = $wpdb;
    $wpdb = $wpdbMongo;
